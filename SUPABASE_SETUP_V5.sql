...
-- Admin-only update/delete/insert for the shared training plan
drop policy if exists plan_workouts_admin_insert on public.plan_workouts;
create policy plan_workouts_admin_insert on public.plan_workouts
for insert to authenticated
with check(exists(select 1 from public.user_settings s where s.user_id=auth.uid() and s.is_admin=true));

drop policy if exists plan_workouts_admin_update on public.plan_workouts;
create policy plan_workouts_admin_update on public.plan_workouts
for update to authenticated
using(exists(select 1 from public.user_settings s where s.user_id=auth.uid() and s.is_admin=true))
with check(exists(select 1 from public.user_settings s where s.user_id=auth.uid() and s.is_admin=true));

drop policy if exists plan_workouts_admin_delete on public.plan_workouts;
create policy plan_workouts_admin_delete on public.plan_workouts
...
-- AFTER you create your account once, run this one-liner separately with your email
-- to enable the in-app Plan Editor:
--
